Meteor.methods({
  ...
  'calendars.insert': function(calendarData, id) {
    if (!Calendars.findOne({"teamid":id})) {
      Calendars.insert(calendarData);
    }
  },

  'calendars.update': function(todoevent, id) {
    Calendars.update({"connectionsid":id}, {$push: { "todoArray": todoevent }});
  },

  'calendars.remove': function(entity, id) {
    Connections.update({"connectionsid":id}, {$pull: { "todoArray": entity }});
  },
  ...
  'forums.insert': function(forum) {
    return Forums.insert(forum);
  },

  'forums.addcomment' (forumId, thismessage) { //this will work if thismessage is set up as messagesArray in txt file
    thismessage.createdAt = new Date();

    if(!Forums.findOne(forumId)){
      throw new Meteor.Error(404, "No such forum");
    }

    Forums.update(forumId, {$push: {commentsArray: thismessage}});
  },

  'forums.deletecomment' (forumId, thismessage) { //this will work if thismessage is set up as messagesArray in txt file
    Forums.update(forumId, {$pull: {commentsArray: thismessage}});
  },
