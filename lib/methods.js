Meteor.methods({
  ...
  'connections.insert': function(connectionsData) {
    if (Connections.find({"connectionsData.connectionsid":Meteor.userId()})) {
      throw new Meteor.Error("connection exists");
    } else {
      Connections.insert(connectionsData);
    }
  },

  'connections.update': function(connectionsData, connection, userId) {
    Connections.update({"connectionsData.connectionid":userId}, {"$push": { "connectionsData.connectionsArray": connection }});
  },
