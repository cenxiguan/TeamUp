...
Router.route('calendars/:_id', {
	...
	template: 'calendars',
	waitOn: function(){
		return [Meteor.subscribe('calendars', this.params._id), Meteor.subscribe('groups'), Meteor.subscribe('user')];
	}
});
...
Router.route('individualforum', {
	...
	waitOn: function(){
		return [Meteor.subscribe('user'),Meteor.subscribe('forums', this.params.query.forumId)];
	},
	data: function(){
...
Router.route('yourForums', {
	waitOn: function(){
		return [Meteor.subscribe('user'),Meteor.subscribe('forums', this.params.query.forumsId)];
	},
});
